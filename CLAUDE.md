...
**Sessions / identity** — `internal/auth` + `internal/httpapi/userauth.go`. One HMAC-signed, HTTP-only cookie (`seeurchin_session`) holds a **map of pollID → session token**, so a browser can hold roles across multiple polls. Participants are still guests created on create/join (the `auth.Provider`/`GuestProvider` seam). **Optional Jellyfin login** (`SEEURCHIN_ENABLE_USER_LOGIN`, default off) authenticates via `jellyfin.AuthenticateByName` and stores a *second* signed cookie (`seeurchin_user`) carrying the identity (`{uid,name,jfAdmin}`). The `requireUser` middleware gates the **NAS-touching actions** — poll creation (`POST /api/polls`) and the write-in request (`request/{id}`) — and is a **no-op when login is disabled**, preserving the guest flow. Hosts who create a poll while signed in get `participants.jellyfin_user_id` populated and a host name defaulted to their Jellyfin name. An optional **per-poll passcode** (`polls.passcode_hash`, an HMAC of the session secret) set at creation gates *joining* one poll (`handleJoin`, constant-time compare); it's orthogonal to Jellyfin login and never touches the NAS.
...
**Admin dashboard** — `internal/httpapi/admin.go` (+ the `/admin` SvelteKit route). Authorized from the logged-in **Jellyfin identity** (no separate admin token): `cfg.AdminEnabled()` is true only when `SEEURCHIN_ENABLE_USER_LOGIN` is on **and** either `SEEURCHIN_ADMIN_USERS` lists ≥1 Jellyfin username (case-insensitive) **or** `SEEURCHIN_ADMIN_JELLYFIN_ADMINS=true` (any account with Jellyfin's `Policy.IsAdministrator`). When disabled every `/api/admin/*` route returns 404; `requireAdmin` otherwise 401s a caller with no `seeurchin_user` cookie and 403s a signed-in non-admin (`isAdminUser`). Sign-in is the shared `/api/user/login`. History reads come from `Service.ListHistory` → `repo.ListPolls` + `repo.AllPollCounts` (grouped queries, no per-poll fan-out). `Service.ForceAdvance` reuses `advanceCore` with `auto=true` (no host check; sparse rounds resolve gracefully). Retention is opt-in (`SEEURCHIN_POLL_RETENTION_DAYS`): `RunRetentionSweeper` (hourly, started in `main` only when > 0) calls `Service.PurgeExpired` → `repo.DeleteClosedPollsBefore`, which keys off the new `polls.closed_at` column (stamped on every close in `CompareAndSetStatus`, since `decided_at` is only set for frozen-winner methods).
